...
namespace System.Text.Lalr
{
    ...
    public class Reporter
    {
        ...
        /// <param name="wantBasis">if set to <c>true</c> [want basis].</param>
        public static void EmitOutput(Context ctx, StreamWriter w, bool wantBasis)
        {
            for (var i = 0; i < ctx.States; i++)
            {
                var states = ctx.Sorted[i];
                w.WriteLine("State {0}:", states.ID);
                var configs = (wantBasis ? states.Basises : states.Configs);
                foreach (var config in configs)
                {
